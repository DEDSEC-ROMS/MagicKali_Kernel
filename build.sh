...
if [[ "$OS" == *"SUSE"* ]] || [[ "$OS" == *"Regata"* ]]; then
  ...
  if [[ ! -d "$AKHILNARANG" ]]; then
      ...
      git clone --depth=1 https://github.com/DEDSEC-ROMS/scripts ${AKHILNARANG}
      cd "${AKHILNARANG}" && bash opensuse.sh
      cd ..
  else
...
if [[ ! -d "$ANYKERNEL3_VERSION" ]]; then
    ...
    git clone --depth=1 https://github.com/DEDSEC-ROMS/AnyKernel3.git "${ANYKERNEL3_VERSION}"
else
    echo -e "$yellow $ANYKERNEL3_VERSION found, skipping step $white"
fi

# Download banner
echo "Do you want to download a insltaller banner?"
select yn in "Yes" "No"; do
    case $yn in
        Yes ) 
            echo "Enter the number of the banner you want to download or type '0' to enter a RAW URL:"
            curl -s https://api.github.com/repos/DEDSEC-ROMS/AnyKernel3_banner/contents/ | grep "name" | awk -F\" '{print NR,$4}'
            read -r banner_number
            if [ "$banner_number" -eq 0 ]; then
                read -p "Enter the RAW URL of the banner you want to download: " banner_url
            else
                banner_name=$(curl -s https://api.github.com/repos/DEDSEC-ROMS/AnyKernel3_banner/contents/ | grep "name" | awk -F\" '{print $4}' | sed "${banner_number}q;d")
                banner_url="https://raw.githubusercontent.com/DEDSEC-ROMS/AnyKernel3_banner/main/$banner_name"
            fi
            wget -O AnyKernel3/banner "$banner_url"
            break;;
        No ) break;;
    esac
done
...
zip_kernelimage() {
    ...
    cd AnyKernel3 || exit
    zip -r9 MagicKali-"${BUILD_TIME}".zip ./*
    cd ..
}
...
Hash() {
    SHA=$(shasum "$(pwd)"/AnyKernel3/MagicKali-"${BUILD_TIME}".zip | cut -f 1 -d '/')
    MD5=$(md5sum "$(pwd)"/AnyKernel3/MagicKali-"${BUILD_TIME}".zip | cut -f 1 -d '/')
}
...
if [ -f "$FILE" ]; then
    ...
    echo "The kernel has successfully been compiled and can be found in $(pwd)/AnyKernel3/MagicKali-${BUILD_TIME}.zip"
